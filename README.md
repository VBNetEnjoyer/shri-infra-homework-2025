...
## Информация
Сервачок крутится тут 👉 http://158.160.147.203:3000/hw/store

Добавил изображение котиков в каталоге, но на карточке товара они не отображаются. Вот бы кто пофиксил...

Для проверки можно написать мне за правами [Telegram](https://t.me/DanMaltsev) или форкнуть и прописать следущие секреты:
- VM_SSH_HOST;
- VM_SSH_KEY;
- VM_SSH_USER;
- YC_OAUTH_TOKEN;
- YC_REGISTRY_ID; (вроде как это уже не нужно, надеюсь я везде убрал его использование из секретов)
